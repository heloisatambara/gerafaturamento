...
try:
    ...
    pool = pd.read_csv("pool_de_horas.csv")
except FileNotFoundError:
    print("Erro: Arquivos Clockify.csv ou pool_de_horas.csv não encontrados.")
    exit()

# Cria dicionário de pool e modelo por cliente
try:
    pool_por_cliente = dict(zip(pool["Cliente"], pool["Pool"]))
    modelo_por_cliente = dict(zip(pool["Cliente"], pool["Horario"]))
except KeyError:
    print("Erro: Colunas 'Cliente', 'Pool' ou 'Horario' não encontradas no arquivo pool_de_horas.csv")
    exit()
...
for cliente in df["Cliente"].unique():
    ...
    # Modelo 8x5
    if modelo_por_cliente[cliente] == "8x5":
